...
midi.loadMidiFile = function(onsuccess, onprogress, onerror, instList) {
	try {
      var instNums = [];
      if (instList) {
         instList.forEach(inst => {
          if (inst)
            instNums.push(MIDI.GM.byName[inst].number);
         else 
            instNums.push(0);
         });  
      }
		midi.replayer = new Replayer(MidiFile(midi.currentData, instNums), midi.timeWarp, null, midi.BPM);
...
midi.loadFile = function(file, onsuccess, instList, onprogress, onerror) {
	midi.stop();
	if (file.indexOf('base64,') !== -1) {
		var data = window.atob(file.split(',')[1]);
		midi.currentData = data;
		midi.loadMidiFile(onsuccess, onprogress, onerror, instList);
	} else {
